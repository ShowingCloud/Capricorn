...
#        self.engine = create_engine( "sqlite:///" + self.record.UUID + '.db')
        ...
        with self.session.begin():
            ...
            parameter.SceneID = self.record.SceneID
    ...
    def next(self):
        with self.sess.begin():
            other = self.sess.query (L_ScenesData).filter_by(UUID = self.record.SceneID).first()
        with self.session.begin():
...
#         #添加到工程库阵地表中
        with self.session.begin():
            ...
            data.Name = self.ui.lineEditName.text()
            data.Location = loca
