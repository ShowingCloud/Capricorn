...
from Models.ProjectDB import IgnitorsData, FieldsData, ScriptData
from PySide.QtCore import Qt, SIGNAL, Slot, QPoint
from PySide.QtGui import QDialog, QMessageBox, QGroupBox, QTableView, \
    QStandardItemModel, QPushButton, QHBoxLayout, QVBoxLayout, QMenu, QAction, \
...
class IgnitionBox(QDialog):
    ...
    def __init__(self, session, UUID, boxUUID, residue,FieldID,musicSignal,parent = None):
...
#            rightMenu.addAction(deleteAction)
    ...
    def addIgnitionBox(self):
        ignitionBoxDatabase = IgnitionBoxDatabase(self.session, self.FieldID,self)
    ...
    def query(self):
        ...
        with self.session.begin():
            row = self.session.query(FieldsData).filter_by(Name = self.FieldID).first()
        with self.session.begin():    
            record = self.session.query(IgnitorsData).filter_by(FieldID = row.UUID)
    ...
    def confirm(self):
        ...
        if len(UUID) > 1:
            ...
            with self.session.begin():
                row1 = self.session.query (ScriptData).filter_by(UUID = self.UUID).first()
            if row1.IgnitorID != 'no choose' and row1.IgnitorID != UUID[0]:
                with self.session.begin():
                    boxRow = self.session.query(IgnitorsData).filter_by(UUID = row1.IgnitorID).first()
                    boxRow.SurplusHeads = boxRow.SurplusHeads + 1
            if row1.IgnitorID == UUID[0]:
                print '******same*******'
                self.accept()
