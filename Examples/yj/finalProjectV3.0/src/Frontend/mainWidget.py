...
#        hbox1.addLayout(vbox1)
    ...
    def changeTypePage(self, current, previous):
        ...
        self.customFireworks.model.setHorizontalHeaderLabels(["UUID", "Name", "Alias", "Size", "Used Effects", "Rising Time", "Stock", "Weight Net", "Weight Gross", "Application", "Safety Distance Horizontal", "Safety Distance Vertical", "Information", "Price", "Notes"])
    ...
    def showScript(self, index):
        #阵地号的UUID
        FieldID = self.fieldList.model.item(index.row(), 1).text()
        self.script.model.clear()
        self.script.model.setHorizontalHeaderLabels(["UUID", "FieldID", "Ignition Time", "Rising Time(s)", "Effect Time", "Name", "Size",  "Ignition ID",  "Tilt Angle", "Information", "Notes"])
        self.script.query(FieldID)
