...
@author: pyroshow
'''
from sqlalchemy import Column, Integer, Sequence, String, create_engine, DateTime, Text, Boolean
...
class FireworksData(base):
    ...
    Size = Column(Integer, nullable = False)   #尺寸(暂定英尺in)数据库里面为mm毫米
    ...
    WeightNet = Column(Integer)                #产品净重，单位k(克)
    WeightGross = Column(Integer)              #产品毛重，单位k(克)
    SDHorizontal = Column(Integer)              #水平安全观看距离，单位m(米)
    SDVertical = Column(Integer)              #水平垂直观看距离，单位m(米)
    EffectID = Column(Integer)              #不清楚，类型待定（燃放的编号）
    ...
    Price = Column(Integer)                #价格，单位待定
    CalcFactor = Column(Integer)           #不清楚，类型待定
    Notes = Column(Text)                 #用户自定义字段，类型待定
    Perm = Column(Integer)                 #0为私有，1为公有，其余均为权限号
    Owner = Column(String)                 #所有者用户ID
    
    def __init__(self, UUID = None, ITime = None, MTime = None,
...
class IgnitorsData(base):
    ...
    def __init__(self, UUID = None, ITime = None, MTime = None, Name = None, Description = None, Picture = None, 
...
class UsersData(base):
    ...
    def __init__(self, UUID = None, ITime = None, MTime = None, UserName = None,FireworkPermList = None,
